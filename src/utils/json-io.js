...
export async function exportGameState(sessionId) {
  ...
  // Note: We deliberately exclude `orama_snapshots` from export payloads.
...
export async function importGameState(fileContent) {
  ...
  if (!isArrayValid(data.turns) || !isArrayValid(data.memory) || !isArrayValid(data.milestones) || !isArrayValid(data.embeddings) || !isArrayValid(data.summary_tasks)) {
