...
export class WizardUI {
  ...
  renderStep() {
    ...
    this.stepLabel.textContent = `Step ${this.wizardState.step} of 5 — ${this.wizardState.step === 1 ? 'Define Your World' : this.wizardState.step === 2 ? 'Protagonist Identity' : this.wizardState.step === 3 ? 'Character Bible' : this.wizardState.step === 4 ? 'Creative Priorities' : 'Finalize The Stage'}`;
    ...
    if (this.wizardState.step === 5) {
      this.nextBtn.innerHTML = `<span>Begin Adventure</span> <i data-lucide="play" class="w-4 h-4"></i>`;
      this.nextBtn.className = 'px-6 py-2 rounded font-medium bg-[var(--success)] text-[var(--bg-base)] hover:opacity-90 shadow flex items-center gap-2';
    } else {
